...
public class Wolf : MonoBehaviour, IInteractable
{
    private enum WolfState { Wandering, Fleeing }
    ...
    private WolfState _currentState = WolfState.Wandering;
    ...
    public void Initialize(SceneBlackboard sceneBlackboard, PlayerDialog playerDialog)
    {
        ...
        _sceneBlackboard.ListenTo(SceneBlackboardKeys.Wolf.Interactable, () =>
            Interactable = _sceneBlackboard.Get<bool>(SceneBlackboardKeys.Wolf.Interactable));

        _sceneBlackboard.ListenTo(SceneBlackboardKeys.Wolf.CanFlee, () =>
            _shouldFlee = _sceneBlackboard.Get<bool>(SceneBlackboardKeys.Wolf.CanFlee));

        _sceneBlackboard.ListenTo(SceneBlackboardKeys.Wolf.CanWander, () =>
            wandering = _sceneBlackboard.Get<bool>(SceneBlackboardKeys.Wolf.CanWander));

        _sceneBlackboard.ListenTo(SceneBlackboardKeys.Wolf.Fed, () =>
        {
            if (_sceneBlackboard.Get<int>(SceneBlackboardKeys.Wolf.Fed) == 0)
                _fed = false;
        });
    ...
    {
        if (!_initialized || _currentState == WolfState.Fleeing)
            return;
        ...
        _fed = true;
        _sceneBlackboard.Set(SceneBlackboardKeys.Wolf.Fed, _sceneBlackboard.Get<int>(SceneBlackboardKeys.Wolf.Fed) + 1);
        _playerDialog.ExecuteDialog("Your ancestors had to hunt for this.");
    }
    ...
    private void EvaluatePlayerAwareness()
    {
        ...
        if (_shouldFlee && _currentFear >= maxFearThreshold && _currentState != WolfState.Fleeing)
        {
            _currentState = WolfState.Fleeing;
    ...
    {
        if (_currentState != WolfState.Fleeing)
            return;
        ...
        if (_fleeTimer <= 0f)
        {
            _currentState = WolfState.Wandering;
            _currentFear = 0f;
    ...
    {
        float currentMoveSpeed = _currentState == WolfState.Fleeing ? runSpeed : walkSpeed;
